...
DenominatorComputation::DenominatorComputation(torch::Tensor forward_transitions,
  ...
  beta_ = torch::zeros({num_sequences_, 2, num_states_ + 1}, torch::kFloat);
...
void DenominatorComputation::AlphaSum(int t) {
...
void DenominatorComputation::AlphaGeneralFrame(int t) {
  ...
  } else
  {
    ...
    auto transition_indices_a = backward_transition_indices_.accessor<int, 2>();
    auto transitions_a = backward_transitions_.accessor<int, 2>();
    auto transition_probs_a = backward_transition_probs_.accessor<float, 1>();

    for (int s = 0; s < num_sequences; s++) {
      for (int h = 0; h < num_hmm_states; h++) {
        float this_tot_alpha = 0.0;
        for (int trans_i = transition_indices_a[h][0];
             trans_i != transition_indices_a[h][1]; trans_i++) {
          float transition_prob = transition_probs_a[trans_i];
          int pdf_id = transitions_a[trans_i][2],
...
void DenominatorComputation::BetaLastFrame() {
  ...
  last_frame_beta.copy_(inv_tot_prob.unsqueeze(1).expand_as(last_frame_beta));
}

void DenominatorComputation::BetaGeneralFrame(int t) {
  assert(t >= 0 && t < num_frames_);
  int num_hmm_states = num_states_,
    num_sequences = num_sequences_,
    num_frames = num_frames_,
  ...
  } else
  {
    ...
    auto transition_indices_a = forward_transition_indices_.accessor<int, 2>();
    auto transitions_a = forward_transitions_.accessor<int, 2>();
    auto transition_probs_a = forward_transition_probs_.accessor<float, 1>();

    for (int h = 0; h < num_hmm_states; h++) {
      for (int s = 0; s < num_sequences; s++) {
        float this_alpha_prob = this_alpha_a[s][h],
	  arbitrary_scale = 1.0 / this_alpha_a[s][num_hmm_states];
        ...
        for (int trans_i = transition_indices_a[h][0]; 
          ...
          float transition_prob = transition_probs_a[trans_i];
          int pdf_id = transitions_a[trans_i][2],
	    next_hmm_state = transitions_a[trans_i][1];
	  ...
	  log_prob_deriv_a[s][pdf_id] += occupation_prob;
        ...
        this_beta_a[s][h] = tot_variable_factor * arbitrary_scale;
      }
    }
  }
}
...
void DenominatorComputation::BetaGeneralFrameDebug(int t) {
  int num_hmm_states = num_states_;
  ...
  float alpha_beta_product = torch::bmm(this_alpha.unsqueeze(1), this_beta.unsqueeze(2)).sum().cpu().data<float>()[0],
  ...
  if (!ApproxEqual(alpha_beta_product, num_sequences_)) {
    std::cerr  << "On time " << t << ", alpha-beta product "
               ...
               << " alpha-sum = " << torch::sum(this_alpha)
               << ", beta-sum = " << torch::sum(this_beta)
               << std::endl;
