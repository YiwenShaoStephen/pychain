...
class DenominatorComputation {
  ...
  */
  DenominatorComputation(torch::Tensor forward_transitions,
 ...
 private:
  ...
  // Note, they are not logs.  The last 'num_sequences'
  // columns, where the alpha for the state indexed 'num_states' would live,
  // are for the alpha-sums, which relates to leaky HMM.
  torch::Tensor alpha_;

  // the beta (also beta-dash) probabilities (rolling buffer); dimension
  // is (num-sequences, 2, num-hmm-states + 1).  [the last
