...
static void _cuda_chain_hmm_forward(const int *backward_transition_indices,
  ...
  // T, H, D are used as strides ,
  int T = num_frames,
    H = num_hmm_states,
    D = num_pdfs;
  
  float this_tot_alpha = 0.0;
  int trans_i = backward_transition_indices[h * 2],
  ...
  for (; trans_i + loop_unroll <= trans_end; trans_i += loop_unroll) {
    float transition_prob0 = backward_transition_probs[trans_i];
    int pdf_id0 = backward_transitions[trans_i * 3 + 2],
        prev_hmm_state0 = backward_transitions[trans_i * 3];
    float transition_prob1 = backward_transition_probs[trans_i + 1];
    int pdf_id1 = backward_transitions[(trans_i + 1) * 3 + 2],
      prev_hmm_state1 = backward_transitions[(trans_i + 1) * 3];
...
static void _cuda_chain_hmm_backward(const int *forward_transition_indices,
  ...
  int trans_i = forward_transition_indices[h * 2],
    trans_end = forward_transition_indices[h * 2 + 1];
  const int loop_unroll = 2;  // don't change this without changing the code
                              // below.
  for (; trans_i + loop_unroll <= trans_end; trans_i += loop_unroll) {
    float transition_prob0 = forward_transition_probs[trans_i];
    int pdf_id0 = forward_transitions[trans_i * 3 + 2],
      next_hmm_state0 = forward_transitions[trans_i * 3 + 1];
    float transition_prob1 = forward_transition_probs[trans_i + 1];
    int pdf_id1 = forward_transitions[(trans_i + 1) * 3 + 2],
      next_hmm_state1 = forward_transitions[(trans_i + 1) * 3 + 1];
